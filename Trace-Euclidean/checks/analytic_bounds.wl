...
symbol["max-g-xk-derivative", D[x k[x], x] - (k[x] - 1), x > 0];
symbol["max-g-xk-second-derivative", D[x k[x], {x, 2}] + 1/x, x > 0];
check[
  "max-g-xk-second-negative",
  "symbolic sign",
  FullSimplify[D[x k[x], {x, 2}] < 0, x >= 3],
  D[x k[x], {x, 2}],
  "negative for x >= 3"
];
...
signCert["max-g-xk-derivative-at-three-negative", is[kI[ip[3]], ip[1]], -1];
...
symbol[
  "stationary-gxx-relation",
  gxx - y gxy/x + y gy/x^2,
  x > 0 && y > 0
];
symbol[
  "stationary-fprime-relation",
  gxy - y gyy/x - D[fStationary[y], y] - gy/x,
  x > 0 && y > 0
];
check[
  "stationary-gxy-negative-from-gxx",
  "exact implication",
  Resolve[
    ForAll[{xx, yy, uxx, uxy},
      Implies[xx > 0 && yy > 0 && uxx < 0 && uxx == yy uxy/xx, uxy < 0]
    ],
    Reals
  ],
  "x,y>0, gxx<0, gxx=(y/x)gxy",
  "gxy<0"
];
check[
  "stationary-hessian-sign",
  "exact implication",
  Resolve[
    ForAll[{xx, yy, uxx, uxy, uyy, fp},
      Implies[
        xx > 0 && yy > 0 && uxx == yy uxy/xx &&
          fp == uxy - yy uyy/xx && uxy < 0 && fp < 0,
        uxx uyy - uxy^2 < 0
      ]
    ],
    Reals
  ],
  "critical-point derivative relations and signs",
  "negative Hessian determinant"
];
