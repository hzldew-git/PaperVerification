...
/-- A primitive vector in a free integral module of positive finite rank
extends to a basis whose zeroth vector is the supplied vector. -/
theorem v15_exists_fin_succ_basis_zero_eq_of_primitive
    {M : Type*} [AddCommGroup M] {n : ℕ}
    (b : Basis (Fin (n + 1)) ℤ M) (v : M)
    (hv : V15IsPrimitiveVector v) :
    ∃ b' : Basis (Fin (n + 1)) ℤ M, b' 0 = v := by
  have hv0 : v ≠ 0 := by
    intro hzero
  ...
  let e : Fin (n + 1) ≃ Fin (n + 1) := Equiv.swap 0 (snf.f 0)
  let b' : Basis (Fin (n + 1)) ℤ M :=
...
/-- A primitive vector in a free rank-two integral module extends to a basis
whose zeroth vector is the supplied vector. -/
theorem v15_exists_fin_two_basis_zero_eq_of_primitive
    {M : Type*} [AddCommGroup M]
    (b : Basis (Fin 2) ℤ M) (v : M)
    (hv : V15IsPrimitiveVector v) :
    ∃ b' : Basis (Fin 2) ℤ M, b' 0 = v := by
  have hv0 : v ≠ 0 := by
    intro hzero
    have hunit : IsUnit (0 : ℤ) := hv 0 0 (by simp [hzero])
    exact not_isUnit_zero hunit
  ...
  have hunit : IsUnit (d * snf.a 0) :=
    hv (d * snf.a 0) (snf.bM (snf.f 0)) hv_bM
  let e : Fin 2 ≃ Fin 2 := Equiv.swap 0 (snf.f 0)
  let b' : Basis (Fin 2) ℤ M :=
    (snf.bM.reindex e).unitsSMul (fun _ ↦ hunit.unit)
  refine ⟨b', ?_⟩
  rw [show b' 0 = (hunit.unit : ℤ) • snf.bM (snf.f 0) by
    simp [b', e, Basis.unitsSMul_apply]]
...
/-- A primitive vector in a free rank-four integral module extends to a basis
whose zeroth vector is the supplied vector. -/
theorem v15_exists_fin_four_basis_zero_eq_of_primitive
    {M : Type*} [AddCommGroup M]
    (b : Basis (Fin 4) ℤ M) (v : M)
    (hv : V15IsPrimitiveVector v) :
    ∃ b' : Basis (Fin 4) ℤ M, b' 0 = v := by
  have hv0 : v ≠ 0 := by
    intro hzero
    have hunit : IsUnit (0 : ℤ) := hv 0 0 (by simp [hzero])
    exact not_isUnit_zero hunit
  ...
  have hunit : IsUnit (d * snf.a 0) :=
    hv (d * snf.a 0) (snf.bM (snf.f 0)) hv_bM
  let e : Fin 4 ≃ Fin 4 := Equiv.swap 0 (snf.f 0)
  let b' : Basis (Fin 4) ℤ M :=
    (snf.bM.reindex e).unitsSMul (fun _ ↦ hunit.unit)
  refine ⟨b', ?_⟩
  rw [show b' 0 = (hunit.unit : ℤ) • snf.bM (snf.f 0) by
    simp [b', e, Basis.unitsSMul_apply]]
