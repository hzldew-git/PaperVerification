...
local notations and the `open ArithmeticFunction` that only the unported remainder used,
replaced project-specific imports by direct Mathlib imports, and moved the retained
declarations into `TraceEuclidean.Analytic`.
Re-ported from the
...
Local adaptation: moved the generic analytic prefix into the TraceEuclidean.Analytic namespace
and retained the upstream instance-priority workaround.
-/
