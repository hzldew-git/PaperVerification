...
Local modifications: added this provenance header and replaced two
documentation-only `axiom` commands by `variable` commands so the repository's
textual forbidden-declaration scan does not misclassify the example.
Re-ported from the
