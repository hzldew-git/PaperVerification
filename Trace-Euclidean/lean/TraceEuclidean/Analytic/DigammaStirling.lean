...
|Im w| ≥ 1, proved from the partial-fraction series (DedekindZeta.DigammaSeries)
...
theorem tendsto_harmonic_sub_clog (hw : 0 < w.re) :
  ...
  have hdecomp : ∀ N : ℕ, (∑ n ∈ Finset.range N, 1 / ((n : ℂ) + 1))
    ...
    have hlog : Complex.log ((N : ℂ) + 1 + w)
      ...
      push_cast
      field_simp
    rw [hlog, DedekindZeta.DigammaSeries.harmonic_cast_eq]
    push_cast
    ring
  simp_rw [hdecomp]
...
theorem digamma_eq (hw : 0 < w.re) (ht : 1 / 2 ≤ |w.im|) :
  ...
  have hL := (DedekindZeta.DigammaSeries.hasSum_digamma_series hmem).tendsto_sum_nat
