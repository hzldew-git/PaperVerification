...
theorem periodisation_eq_tsum_fourier (x : V) :
  ...
  have hcoeff : ∀ w : dualLattice L,
    ...
    have hgcont : Continuous
      ...
      have hprod : Continuous (fun θ : Fin d → AddCircle (1 : ℝ) =>
          ∏ i, (starRingEnd ℂ) (fourier (dualLatticeEquiv L b w i) (θ i))) := by
        simpa using
          (continuous_finsetProd (s := Finset.univ)
            (f := fun i : Fin d => fun θ : Fin d → AddCircle (1 : ℝ) =>
              (starRingEnd ℂ) (fourier (dualLatticeEquiv L b w i) (θ i))) (by
            intro i _
