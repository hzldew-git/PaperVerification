...
-/
/-
DedekindZeta/DigammaSeries.lean — the digamma partial-fraction series.

Modified for this project: imports were reduced to the required mathlib files,
and declarations were placed in the `DedekindZeta.DigammaSeries` namespace.

Target:  digamma z = −γ − 1/z + ∑'_{n≥0} (1/(n+1) − 1/(z+n+1))   for z ∈ ℂ_ℤ,
the Mathlib-missing piece needed for the remaining H-Γ fields
([eq:mufacts]; adapted from anthropics/formal-math at commit
fbdc36bbf17d20af3fd0447c6d1a8a02773c9844). Route (modelled on Mathlib's
...
/-- Complex conjugation commutes with digamma away from its poles. -/
theorem digamma_conj {z : ℂ} (hz : z ∈ Complex.integerComplement) :
  ...
  have hz' : starRingEnd ℂ z ∈ Complex.integerComplement := by
    ...
    apply hz
    refine ⟨k, ?_⟩
    have h := congrArg (starRingEnd ℂ) hk
    simpa using h
  have h1 := hasSum_digamma_series hz
  have h2 := hasSum_digamma_series hz'
  have h1c : HasSum
      (fun n : ℕ => 1 / ((n : ℂ) + 1) -
        1 / (starRingEnd ℂ z + n + 1))
      (starRingEnd ℂ
        (Complex.digamma z + (Real.eulerMascheroniConstant : ℂ) + 1 / z)) := by
    have h := (Complex.hasSum_conj').mpr h1
    refine h.congr_fun fun n => ?_
    simp only [map_sub, map_div₀, map_one, map_add, map_natCast]
  have huniq := h2.unique h1c
  have heq : starRingEnd ℂ
        (Complex.digamma z + (Real.eulerMascheroniConstant : ℂ) + 1 / z) =
      starRingEnd ℂ (Complex.digamma z) +
        (Real.eulerMascheroniConstant : ℂ) + 1 / starRingEnd ℂ z := by
