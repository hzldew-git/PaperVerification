...
theorem setLIntegral_cone_eq_collar_radial (h : mixedSpace K → ℝ≥0∞)
  ...
  calc
    ...
    _ = ∫⁻ t in Set.Ioi (0 : ℝ),
        ...
        have hmul := lintegral_mul_const
          (μ := (DedekindZeta.mixedMulHaar K).restrict (mixedEmbedding.fundamentalCone K))
          (ENNReal.ofReal t⁻¹) (hh.mul hindmeas)
    ...
    _ = ∫⁻ y in mixedEmbedding.fundamentalCone K,
        ...
        have hconst := lintegral_const_mul (μ := volume.restrict (Set.Ioi (0 : ℝ)))
          (h y) ((hg1_meas (mixedEmbedding.norm y)).mul
            (measurable_inv.ennreal_ofReal))
        simp only [Pi.mul_apply] at hconst
        rw [hconst]
        congr 1
...
theorem setLIntegral_cone_eq_radial (h : mixedSpace K → ℝ≥0∞) (hh : Measurable h) :
  ...
  have hinner : ∀ r : ℝ, 0 ≤ r →
    ...
    have hgσ : Measurable (fun σ : mixedSpace K => h (radialMap K r σ)) := by
      have hcont : Continuous (fun σ : mixedSpace K => radialMap K r σ) :=
        continuous_radialMap_uncurry.comp (continuous_const.prodMk continuous_id)
    ...
    rw [lintegral_map
      (μ := (DedekindZeta.mixedMulHaar K).restrict
        (mixedEmbedding.fundamentalCone K ∩
          {x | mixedEmbedding.norm x ∈ Set.Ioc 1 (Real.exp 1)}))
      hgσ hradialProj]
...
theorem mixedMulHaar_restrict_cone_eq_map :
  ...
  have hTonelli :
      (∫⁻ a : ℝ × mixedSpace K,
        ENNReal.ofReal a.1⁻¹ * h (radialMap K a.1 a.2)
          ∂(volume.restrict (Set.Ioi (0 : ℝ))).prod (surfaceMeasure K))
        = ∫⁻ r in Set.Ioi (0 : ℝ), ∫⁻ σ : mixedSpace K,
          ENNReal.ofReal r⁻¹ * h (radialMap K r σ) ∂(surfaceMeasure K) ∂volume := by
    simpa only [Pi.mul_apply] using
      (lintegral_prod (μ := volume.restrict (Set.Ioi (0 : ℝ)))
        (ν := surfaceMeasure K)
        (fun a : ℝ × mixedSpace K => ENNReal.ofReal a.1⁻¹ * h (radialMap K a.1 a.2))
  ...
  have hmσ : Measurable (fun σ : mixedSpace K => h (radialMap K r σ)) :=
    hh.comp ((continuous_radialMap_uncurry.comp
      (continuous_const.prodMk continuous_id)).measurable)
  have hconst := lintegral_const_mul (μ := surfaceMeasure K)
    (ENNReal.ofReal r⁻¹) hmσ
  rw [hconst, Measure.restrict_eq_self_of_ae_mem hae, mul_comm]
