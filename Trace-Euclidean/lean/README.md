...
| V15DedekindZetaJensenCount | Actual zero occurrences bounded by divisor multiplicities and Jensen's inequality; an explicit quadratic circle-growth premise gives the coarse count, direct zero-sum convergence, and a conditional Table 4 bridge without HSW's sharp count |
| DedekindZeta/*, V15DedekindZetaConstructed | Vendored number-field theta/Poisson and Mellin bridge, global entire continuation of `(s-1) ζ_K(s)`, and a concrete instance for the zero theory; the functional equation and growth bound remain open |
