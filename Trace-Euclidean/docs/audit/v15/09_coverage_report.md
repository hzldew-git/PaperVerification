...
| Odlyzko `b=4` analytic kernel | Complete elementary Fourier layer | Lean proves the kernel properties, the autocorrelation formula for `H`, exact Fourier scaling, the exact transform `2*pi/cosh(2*pi^2*w)` of the hyperbolic-secant factor, the product-to-convolution identity, and a real nonnegative Fourier transform for the complete `F`. The zeta explicit formula and certified A/B estimates remain external. |
