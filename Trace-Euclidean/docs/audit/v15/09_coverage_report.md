...
| Odlyzko `b=4` analytic kernel | Fourier, zero-transform, `C^4` gluing, exact error integral, completed-zeta growth, zero count, and conditional source formula | Lean proves the kernel properties, the autocorrelation formula for `H`, exact Fourier scaling, the hyperbolic-secant and tilted-secant transforms, convolution positivity for `F`, and `Re Phi(s) >= 0` for every `0 <= Re(s) <= 1`, including both boundaries. It proves `Phi(1-s) = Phi(s)`, `Phi(0) = Phi(1) = 16/3`, global `C^4` regularity, uniform fourth-power transform decay, `4 * integral_(0,infinity) F(x)*cosh(x/2) dx = 32/3`, and convergence of both source archimedean integrals. It constructs the completed zeta, proves its functional equation and quadratic exponential growth, derives a quadratic count of actual zero occurrences by Jensen, and proves absolute convergence of the direct zero sum. A source-exact conditional theorem now derives Table 4 with the Stark/Weil explicit formula as its sole mathematical premise. The strict A/B estimates use a disclosed native_decide compiler trust boundary. |
