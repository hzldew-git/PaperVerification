...
| Odlyzko `b=4` analytic kernel | Fourier, zero-transform, differentiability, exact error integral, and conditional source formula | Lean proves the kernel properties, the autocorrelation formula for `H`, exact Fourier scaling, the hyperbolic-secant and tilted-secant transforms, convolution positivity for `F`, and `Re Phi(s) >= 0` for every `0 <= Re(s) <= 1`, including both boundaries. It proves `Phi(1-s) = Phi(s)`, `Phi(0) = Phi(1) = 16/3`, global differentiability, derivative decay, `4 * integral_(0,infinity) F(x)*cosh(x/2) dx = 32/3`, and convergence of the cosh-denominator source integral. A summable zero family has nonnegative total real part. A source-exact conditional theorem derives Table 4 from the explicit formula and strict archimedean certificate. The zeta explicit formula, paired-zero convergence, sinh-denominator convergence, and certified A/B estimates remain external. |
