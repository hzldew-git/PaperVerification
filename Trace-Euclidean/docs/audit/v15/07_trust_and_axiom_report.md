...
The source-exact equation (2.3) and strict `A,B` integral bounds are named
unproved premises in a separate conditional theorem that derives the Table 4
interface. The cosh-denominator integral is proved convergent; sinh-denominator
convergence remains a premise. These remaining inputs are not project axioms
or proved source theorems.
