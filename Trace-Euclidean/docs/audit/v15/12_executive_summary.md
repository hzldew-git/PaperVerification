...
All four reviewed main results are PROVISIONAL_MATCH; none is independently VERIFIED_MATCH. The generic covering-radius formula of Proposition 6.1 is now proved on the real and rational planes and transferred to every qualifying real-quadratic fractional ideal, together with its determinant identity. The canonical abstract rank-one-to-ideal presentation is also exported in Lean. Remaining limits include the cited external Odlyzko theorem, the two integer-scalar specializations not bundled in a single proposition-level endpoint, selected supporting lemmas, and unsigned author/domain/Lean review cards.

The complete Lean build and expanded 89-endpoint axiom audit pass. The unchanged v15 Python/Wolfram checks passed in the documented reruns. The v9 release evidence is retained as a historical version and is not counted as v15 evidence.
