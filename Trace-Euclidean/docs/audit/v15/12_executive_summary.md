...
timestamps and SHA-256 digests. A reproducible generator verifies the digests,
row structure, polynomial degrees, monicity, counts, first discriminants, and
sorted order before producing the Lean discriminant columns. Lean checks the
five imported column lengths and sorted order, proves their minima, and reduces
both the degree 5--9 minima and degree-ten exclusion to the single
source-facing statement `V15VoightEnumerationUpToFourteenInput`. The strongest
Section 4 endpoints now require that completeness statement and the optimized
degree-eleven bound. The finite tables do not by themselves prove completeness
of Voight's enumeration algorithm.
