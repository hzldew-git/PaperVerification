...
The complete 8836-job local Lean build passes. The 346-declaration main axiom audit includes the quadratic discriminant, constructed zeta regularization, functional equation, Mellin growth, Jensen, finite contour, endpoint, archimedean shift, vertical limit, and transformed prime-power endpoints using only `propext`, `Classical.choice`, and `Quot.sound`; the separate 17-declaration numerical audit exposes exactly the A/B interval certificate's five disclosed native-compiler dependencies. The four finite-array equality proofs are kernel computations with no axiom dependencies. Local public reruns produced 1156 Python PASS and 115 Wolfram PASS, both with zero failures. Superseded release evidence is not included in the current package or v15 counts.
