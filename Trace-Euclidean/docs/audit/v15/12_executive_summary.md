...
column projections, and maximum indices. A kernel bridge sends every valid row
into the exact Hunter discriminant filter from the explicitly named
`V15VoightPolynomialDiscriminantInput`. Lean proves the column minima and
reduces both the degree 5--9 minima and degree-ten exclusion to
`V15VoightEnumerationUpToFourteenInput`. The finite tables do not by themselves
prove the 2,773 coefficient-discriminant equations in the Lean kernel or the
completeness of Voight's enumeration algorithm.
