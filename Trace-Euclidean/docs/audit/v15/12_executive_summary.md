...
filtered box.
Exhausting the large boxes, proving total reality for the archived rows inside
Lean, checking the resulting orders and duplicate fields, and handling the
...
The complete local 8,959-job Lean build passes. The 562-declaration main axiom audit includes the unconditional quartic short-vector selection, general Hunter coefficient and discriminant bridges, refined and discriminant-filtered finite degree-five and degree-seven boxes, the generic Bareiss/resultant/discriminant and finite-field irreducibility semantics, the full Voight polynomial-row bridge, and degree-eleven sensitivity checks; every kernel-only endpoint uses only a subset of `propext`, `Classical.choice`, and `Quot.sound`. Final endpoints that use the strict Odlyzko numerical certificate show its disclosed native-compiler dependencies. The separate 57-declaration numerical audit isolates the Voight finite-data, resultant, and irreducibility certificate dependencies and their propagation. The finite quartic searches use kernel `decide` and add no compiler trust. All three Voight generator checks pass. Local public reruns produced 1156 Python PASS, 115 Wolfram classification PASS, and a 2,773-row Voight Mathematica PASS, all with zero failures. Superseded release evidence is not included in the current package or v15 counts.
